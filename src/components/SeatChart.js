...
const SeatChart = ({ occasion, tokenAccess, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false);
  const [hasSold, setHasSold] = useState(false);

  const getSeatsTaken = async () => {
    const seatsTaken = await tokenAccess.getSeatsTaken(occasion.id);
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await tokenAccess
      .connect(signer)
